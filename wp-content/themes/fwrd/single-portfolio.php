							...
							<div class="portfolio-rightside">
								...
								<?php
									...
									if(!empty($clients)){
										if(count($clients) > 1){
										?>
											<div class="portfolio-sidetitle"><?php esc_html_e("Clients", 'fwrd')?></div>
										<?php
										}else{
										?>
											<div class="portfolio-sidetitle"><?php esc_html_e("Client", 'fwrd')?></div>
										<?php
										}
										?>
								...
								if(!empty($terms)){
								?>
								<div class="portfolio-sidetitle"><?php esc_html_e("Categories", 'fwrd')?></div>
								<div class="portfolio-sidesplit"></div>
								<?php
									$first = 1;
									foreach($terms as $term){
										if($first == 1){
											echo '<a href="'.esc_url("../../portfolio-category/".$term).'">'.$term.'</a>';
											$first = 0;
										}else{
											echo ', '.'<a href="'.esc_url("../../portfolio-category/".$term).'">'.$term.'</a>';
										}
								...
								<?php
									...
									if(!empty($externallabel) && !empty($externallink)){
										echo '<a class="portfolio-button" href="'.esc_url($externallink).'" target="'.$externaltarget.'">'.$externallabel.'</a>';
									}
								?>  
							</div>
							<div class="clear"></div>
						...
						<div class="portfolio-nav-wrap">
							...
							if (!empty( $prev_post )): ?>
								<a href="<?php echo get_permalink($prev_post->ID); ?>" class="portfolio-prev">
									...
									<div class="prev-text"><?php echo esc_html__("Previous project", 'fwrd'); ?></div>
									<div class="clear"></div>
								</a>
							<?php endif; ?>			
							</div>
							...
							if (!empty( $next_post )): ?>
								<a href="<?php echo get_permalink($next_post->ID); ?>" class="portfolio-next">
									...
									<div class="next-text"><?php echo esc_html__("Next project", 'fwrd'); ?></div>
									<div class="clear"></div>
								</a>
							<?php endif; ?>
							</div>
