 ...
 */
$title = $source = $image = $img_fullwidth = $custom_src = $onclick = $img_size = $external_img_size = $caption = $img_link_large = $link = $img_link_target = $alignment = $el_class = $css_animation = $style = $external_style = $border_color = $css = '';
...
$wrapperClass = 'vc_single_image-wrapper ' . $img_fullwidth . $style . ' ' . $border_color;
...
if ( $img == null ) {
	$img['thumbnail'] = '<img class="vc_img-placeholder vc_single_image-img '.$img_fullwidth.'" src="' . vc_asset_url( 'vc/no_image.png' ) . '" />';
}//' <small>'.esc_html__('This is image placeholder, edit your page to replace it.', 'js_composer').'</small>';
$el_class = $this->getExtraClass( $el_class );
...
$image_string = ! empty( $link_to ) ? '<a' . $a_class . ' href="' . esc_url($link_to) . '"' . ' target="' . $img_link_target . '"' . '><div class="vc_single_image-wrapper ' . $style . ' ' . $border_color . '">' . $img_output . '</div></a>' : '<div class="vc_single_image-wrapper ' . $img_fullwidth . ' ' . $style . ' ' . $border_color . '">' . $img_output . '</div>';
