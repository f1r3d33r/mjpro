...
function acf_form( $options = array() )
{
	...
	$defaults = array(
		...
		'html_before_fields' => '',
		'html_after_fields' => '',
		'submit_value' => esc_html__("Update", 'acf'),
		'updated_message' => esc_html__("Post updated", 'acf'), 
	);
	...
	if( is_array($acfs) ){ foreach( $acfs as $acf ){
		...
		echo '<div id="acf_' . esc_attr($acf['id']) . '" class="postbox acf_postbox ' . $acf['options']['layout'] . '">';
		echo '<h3 class="hndle"><span>' . esc_attr($acf['title']) . '</span></h3>';
