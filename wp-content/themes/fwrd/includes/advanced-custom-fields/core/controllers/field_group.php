...
class acf_field_group
{
	...
	*/
	...
	function admin_head()
	{
		...
		// l10n
		$l10n = array(
			'move_to_trash'		=>	esc_html__("Move to trash. Are you sure?",'acf'),
			'checked'			=>	esc_html__("checked",'acf'),
			'no_fields'			=>	esc_html__("No toggle fields available",'acf'),
			'title'				=>	esc_html__("Field group title is required",'acf'),
			'copy'				=>	esc_html__("copy",'acf'),
			'or'				=>	esc_html__("or",'acf'),
			'fields'			=>	esc_html__("Fields",'acf'),
			'parent_fields'		=>	esc_html__("Parent fields",'acf'),
			'sibling_fields'	=>	esc_html__("Sibling fields",'acf'),
			'hide_show_all'		=>	esc_html__("Hide / Show All",'acf')
		);
...
(function($) {
	...
	acf.l10n = <?php echo json_encode( $l10n ); ?>;
		...
		// add metaboxes
		add_meta_box('acf_fields', esc_html__("Fields",'acf'), array($this, 'html_fields'), 'acf', 'normal', 'high');
		add_meta_box('acf_location', esc_html__("Location",'acf'), array($this, 'html_location'), 'acf', 'normal', 'high');
		add_meta_box('acf_options', esc_html__("Options",'acf'), array($this, 'html_options'), 'acf', 'normal', 'high');
	...
	function ajax_render_location( $options = array() )
	{
		...
		switch($options['param'])
		{
			...
			case "page_type" :
				...
				$choices = array(
					'front_page'	=>	esc_html__("Front Page",'acf'),
					'posts_page'	=>	esc_html__("Posts Page",'acf'),
					'top_level'		=>	esc_html__("Top Level Page (parent of 0)",'acf'),
					'parent'		=>	esc_html__("Parent Page (has children)",'acf'),
					'child'			=>	esc_html__("Child Page (has parent)",'acf'),
				);
			...
			case "page_template" :
				...
				$choices = array(
					'default'	=>	esc_html__("Default Template",'acf'),
				);
			...
			case "post_status" :
				...
				$choices = array(
					'publish'	=> esc_html__( 'Publish', 'acf'),
					'pending'	=> esc_html__( 'Pending Review', 'acf' ),
					'draft'		=> esc_html__( 'Draft', 'acf' ),
					'future'	=> esc_html__( 'Future', 'acf' ),
					'private'	=> esc_html__( 'Private', 'acf' ),
					'inherit'	=> esc_html__( 'Revision', 'acf' ),
					'trash'		=> esc_html__( 'Trash', 'acf' )
				);
			...
			case "user_type" :
				...
				if( is_multisite() )
				{
					$choices['super_admin'] = esc_html__('Super Admin', 'acf');
				}
			...
			case "ef_taxonomy" :
				...
				$choices = array('all' => esc_html__('All', 'acf'));
			...
			case "ef_user" :
				...
				$choices = array_merge( array('all' => esc_html__('All', 'acf')), $wp_roles->get_names() );
