...
class acf_field_taxonomy extends acf_field
{
	...
	*/
	...
	function __construct()
	{
		// vars
		$this->name = 'taxonomy';
		$this->label = esc_html__("Taxonomy",'acf');
		$this->category = esc_html__("Relational",'acf');
		$this->defaults = array(
			'taxonomy' 			=> 'category',
			'field_type' 		=> 'checkbox',
			'allow_null' 		=> 0,
			...
			<?php if( $field['allow_null'] ): ?>
				<li>
					<label class="selectit">
						<input type="<?php echo $field['field_type']; ?>" name="<?php echo $field['name']; ?>" value="" /> <?php esc_html_e("None", 'acf'); ?>
					</label>
				</li>
			<?php endif; ?>
...
<tr class="field_option field_option_<?php echo $this->name; ?>">
	...
	<td>
		...
		do_action('acf/create_field', array(
			...
			'optgroup' => true,
			'choices' => array(
				esc_html__("Multiple Values",'acf') => array(
					'checkbox' => esc_html__('Checkbox', 'acf'),
					'multi_select' => esc_html__('Multi Select', 'acf')
				),
				esc_html__("Single Value",'acf') => array(
					'radio' => esc_html__('Radio Buttons', 'acf'),
					'select' => esc_html__('Select', 'acf')
				)
			)
		));
		?>
...
<tr class="field_option field_option_<?php echo $this->name; ?>">
	...
	<td>
		...
		do_action('acf/create_field', array(
			...
			'message' => esc_html__("Load value based on the post's terms and update the post's terms on save",'acf')
		));
		?>
	</td>
</tr>
<tr class="field_option field_option_<?php echo $this->name; ?>">
	...
	<td>
		...
		do_action('acf/create_field', array(
			...
			'layout'	=>	'horizontal',
			'choices'	=> array(
				'object'	=>	esc_html__("Term Object",'acf'),
				'id'		=>	esc_html__("Term ID",'acf')
			)
		));
		?>
	</td>
