...
class acf_field_relationship extends acf_field
{
	...
	*/
	...
	function __construct()
	{
		...
		$this->label = esc_html__("Relationship",'acf');
		...
		$this->l10n = array(
			'max'		=> esc_html__("Maximum values reached ( {max} values )",'acf'),
			'tmpl_li'	=> '
							<li>
	...
	*/
	...
	function get_result( $post, $field, $the_post, $options = array() ) {
		...
		if( $post_title === '' ) {
			...
			$post_title = esc_html__('(no title)', 'acf');
	...
	<div class="relationship_left">
		<table class="widefat">
			<thead>
				...
				<tr>
					<th>
						<input class="relationship_search" placeholder="<?php esc_html_e("Search...",'acf'); ?>" type="text" id="relationship_<?php echo $field['name']; ?>" />
					</th>
				</tr>
				<?php endif; ?>
				...
				<tr>
					<th>
						...
						// vars
						$choices = array(
							'all' => esc_html__("Filter by post type",'acf')
						);
...
<tr class="field_option field_option_<?php echo $this->name; ?>">
	...
	<td>
		...
		do_action('acf/create_field', array(
			...
			'layout'	=>	'horizontal',
			'choices'	=> array(
				'object'	=>	esc_html__("Post Objects",'acf'),
				'id'		=>	esc_html__("Post IDs",'acf')
			)
		));
		?>
	</td>
...
<tr class="field_option field_option_<?php echo $this->name; ?>">
	...
	<td>
		...
		do_action('acf/create_field', array(
			...
			'value'	=>	$field['filters'],
			'choices'	=>	array(
				'search'	=>	esc_html__("Search",'acf'),
				'post_type'	=>	esc_html__("Post Type Select",'acf'),
			)
		));
		?>
	</td>
	...
	<td>
		...
		do_action('acf/create_field', array(
			...
			'choices' => array(
				...
				'post_title' => esc_html__("Post Title",'acf'),
