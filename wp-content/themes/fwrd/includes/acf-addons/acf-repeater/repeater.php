...
class acf_field_repeater extends acf_field
{
	...
	*/
	...
	function __construct()
	{
		// vars
		$this->name = 'repeater';
		$this->label = esc_html__("Repeater",'acf');
		$this->category = esc_html__("Layout",'acf');
		$this->defaults = array(
			'sub_fields'	=>	array(),
			'row_min'		=>	0,
			'row_limit'		=>	0,
			'layout' 		=> 'table',
			'button_label'	=>	esc_html__("Add Row",'acf'),
		);
		$this->l10n = array(
			'min'	=>	esc_html__("Minimum rows reached ( {min} rows )",'acf'),
			'max'	=>	esc_html__("Maximum rows reached ( {max} rows )",'acf'),
		);
