...
if ( ! class_exists( 'acf_field_widget_area' ) ) {
	...
	class acf_field_widget_area extends acf_field {
		...
		function __construct() {
			...
			$this->category = esc_html__( 'Content', 'acf' ); // Basic, Content, Choice, etc
			$this->defaults = array(
				'allow_null' => 0,
			);
		...
		function create_options( $field ) {
			...
			<tr class="field_option field_option_<?php echo esc_attr( $this->name ); ?>">
				<td class="label">
					<label><?php esc_html_e( "Display Widget Area HTML or Return Widget Area Name", 'acf_widget_area' ); ?></label>
				</td>
				<td>
					<?php
					do_action( 'acf/create_field', array(
