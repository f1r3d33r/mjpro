...
if ( ! class_exists( 'acf_5_field_widget_area' ) ) {
	...
	class acf_5_field_widget_area extends acf_field {
		...
		function __construct() {
			 ...
			 */
			$this->label = esc_html__( 'Widget Area', 'acf_widget_area' );
			 ...
			 *  var message = acf.esc_html_e('FIELD_NAME', 'error');
			 */
			$this->l10n = array();
		...
		function render_field_settings( $field ) {
			...
			acf_render_field_setting( $field, array(
				'label'        => esc_html__( "Allow Null?", 'acf' ),
				'type'         => 'radio',
				'name'         => 'allow_null',
				'choices' => array(
			...
			acf_render_field_setting( $field, array(
				'label'        => esc_html__( "Display Widget Area HTML or Return Widget Area Name", 'acf_widget_area' ),
				'type'         => 'radio',
				'name'         => 'display_or_return',
				'choices' => array(
					'display' => esc_html__( "Display Widget Area HTML", 'acf_widget_area' ),
					'return'  => esc_html__( "Return Widget Name", 'acf_widget_area' ),
				),
				'layout'  => 'horizontal',
			) );
		...
		function render_field( $field ) {
			...
			// null
			if ( $field['allow_null'] ) {
				echo '<option value="null">' . esc_html_x( '- Select - ','ACF Widget Area Null Select Option', 'acf_widget_area' ) . '</option>';
			}
